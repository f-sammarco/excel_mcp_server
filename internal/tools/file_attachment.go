...
//
// Handing over the file is the whole point of the call that gets here, so a failure
// to read it is an error result rather than a note on a success — a caller told
// "exported" with nothing attached would go on believing the file had arrived.
func AttachWorkbookFile(result *mcp.CallToolResult, fileAbsolutePath string) *mcp.CallToolResult {
	...
	if !ok {
		return attachmentError("unsupported file extension: %s", ext)
	}

	info, err := os.Stat(fileAbsolutePath)
	if err != nil {
		return attachmentError("%s", err.Error())
	}
	if info.Size() > attachmentMaxBytes {
		return attachmentError(
			"the file is %d bytes, which exceeds the %d byte attachment limit",
...
func attachmentError(format string, args ...any) *mcp.CallToolResult {
	return mcp.NewToolResultError("File not attached: " + fmt.Sprintf(format, args...))
}
